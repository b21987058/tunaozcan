...
void showDoubleMatrix( double** matrix, int rows, int cols) {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            std::cout << matrix[i][j] << "\t"; // Her eleman aras�na bir sekme (tab) b�rakarak g�r�nt�leme
        }
        std::cout << std::endl; // Her sat�r sonunda yeni bir sat�r ba�lat
    }
}
